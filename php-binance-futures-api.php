<?php
/*
 * @inspired on package php-binance-api
 * @link https://github.com/jaggedsoft/php-binance-api
 * ============================================================
 * @copyright 2020-2021
 * @author Mark Ivan Eubans
 * @version 1
 * ============================================================
 * A curl HTTP REST wrapper for the binance futures currency exchange
 */
...
class BinanceFuturesAPI
{
    ...
    protected $baseTestnet = 'https://testnet.binancefuture.com/fapi/'; // /< Testnet REST endpoint for the currency exchange
    protected $futuresData = 'https://fapi.binance.com/futures/'; // /< REST endpoint for futures trading data
    protected $stream = 'wss://fstream.binance.com/ws/'; // /< Endpoint for establishing websocket connections
    protected $streamTestnet = 'wss://stream.binancefuture.com/ws/'; // /< Testnet endpoint for establishing websocket connections
    ...
    protected $useTestnet = false; // /< Enable/disable testnet (https://testnet.binancefuture.com/fapi/)
    ...
    public function __construct()
    {
        ...
        switch (count($param)) {
            case 0:
                ...
                // @TODO removal of option to setup from file

                break;
            case 1:
                $this->useTestnet = (bool)$param[0];
                // override testnet
                // $this->useTestnet = true;
    ...
    public function history(string $symbol, int $limit = 500, int $fromTradeId = -1)
    {
        ...
        return $this->httpRequest("v1/userTrades", "GET", $parameters, true);
    }
    ...
    {
        return $this->httpRequest("v2/account", "GET", [], true);
    }
    ...
    protected function httpRequest(string $url, string $method = "GET", array $params = [], bool $signed = false)
    {
        ...
        if ($signed === true) {
            ...
            // Check if futures/data is true
            if (isset($params['futures'])) {
                if ($this->useTestnet) {
                    return "futures data endpoints are not available in testnet";
                }
                unset($params['futures']);
                $base = $this->futuresData;
            }
    ...
    // FUTURES ORDER NEW
    public function futuresOrder(string $side, string $symbol, $quantity, string $type = "LIMIT", $price = false, array $flags = [], bool $test = false)
    {
        $opt = [
            "symbol" => $symbol,
            "side" => $side,
        ...
        error_log(json_encode($opt), 0);

        $qstring = ($test === false) ? "v1/order" : "v1/order/test";
        return $this->httpRequest($qstring, "POST", $opt, true);
    }

    public function orderBackup(string $side, string $symbol, $quantity, $price, string $type = "LIMIT", array $flags = [], bool $test = false)
    {
        $opt = [
            "symbol" => $symbol,
            "side" => $side,
    ...
    {
        $arr = array();
        $api_status = $this->httpRequest("v1/ping", 'GET');
        if ( empty($api_status) ) {
    ...
    * - NEXT_QUARTER
     ...
     */
    public function continuousContractKLines($pair, $contractType, $interval, array $flags = [])
    {
        $opt = [
            "pair" => $pair,
        ...
        if (isset($flags['startTime'])) {
            $opt['startTime'] = $flags['startTime'];
        }

        if (isset($flags['endTime'])) {
            $opt['endTime'] = $flags['endTime'];
        }

        // Default 500; max 1500.
        if (isset($flags['limit'])) {
            $opt['limit'] = $flags['limit'];
        }

        return $this->httpRequest("v1/indexPriceKlines", "GET", $opt);
    }
     ...
     * @todo create documentation here
     */
    public function indexPriceKline($pair, $interval, array $flags = [])
    {
        $opt = [
            "pair" => $pair,
            "interval" => $interval,
     ...
     * @todo create documentation here
     */
    public function markPriceKline($symbol, $interval, array $flags = [])
    {
        $opt = [
            "symbol" => $symbol,
            "interval" => $interval,
        ...
        return $this->httpRequest("v1/markPriceKlines", "GET", $opt);
    }
     ...
     * @todo create documentation here
     */
    public function markPrice(array $flags = [])
    {
        $opt = [];

        if (isset($flags['symbol'])) {
            $opt['symbol'] = $flags['symbol'];
        }

        return $this->httpRequest("v1/premiumIndex", "GET", $opt);
    }
     ...
     * @todo create documentation here
     */
    public function getFundingRateHistory(array $flags = [])
    {
        $opt = [];
        ...
        return $this->httpRequest("v1/fundingRate", "GET", $opt);
    }
    ...
    public function openInterest($symbol)
    {
        ...
        return $this->httpRequest("v1/openInterest", "GET", $opt);
    }
     ...
     * @todo create documentation here
     */
    public function openInterestStatistics($symbol, $period)
    {
        $opt = [
            "symbol" => $symbol,
            "period" => $period,
        ...
        return $this->httpRequest("data/openInterestHist", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function topTraderLongShortAccountRatio($symbol, $period)
    {
        $opt = [
            "symbol" => $symbol,
            "period" => $period,
        ...
        return $this->httpRequest("data/topLongShortAccountRatio", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function topTraderLongShortPositionRatio($symbol, $period)
    {
        $opt = [
            "symbol" => $symbol,
            "period" => $period,
        ...
        return $this->httpRequest("data/topLongShortPositionRatio", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function globalLongShortAccountRatio($symbol, $period)
    {
        $opt = [
            "symbol" => $symbol,
            "period" => $period,
        ...
        return $this->httpRequest("data/globalLongShortAccountRatio", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function takerLongShortRatio($symbol, $period)
    {
        $opt = [
            "symbol" => $symbol,
            "period" => $period,
        ...
        return $this->httpRequest("data/takerlongshortRatio", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function lvtKlines($symbol, $interval)
    {
        $opt = [
            "symbol" => $symbol,
            "interval" => $interval
        ...
        return $this->httpRequest("v1/lvtKlines", "GET", $opt);
    }
     ...
     * @todo create documentation here
     */
    public function indexInfo(array $flags = [])
    {
        $opt = [];
        ...
        return $this->httpRequest("v1/indexInfo", "GET", $opt);
    }

    /**
     * incomeType = "TRANSFER"，"WELCOME_BONUS", "REALIZED_PNL"，"FUNDING_FEE", "COMMISSION" and "INSURANCE_CLEAR"
     * @todo create documentation here
     */
    public function getIncomeHistory(array $flags = [])
    {
        $opt = [];
        ...
        if (isset($flags['incomeType'])) {
            $opt['incomeType'] = $flags['incomeType'];
        }
        ...
        return $this->httpRequest("v1/income", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function positionInformation(array $flags = [])
    {
        $opt = [];
        ...
        return $this->httpRequest("v2/positionRisk", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function newFutureAccountTransfer(array $flags = [])
    {
        $opt = [];
     ...
     * @todo create documentation here
     */
    public function accountBalance(array $flags = [])
    {
        $opt = [];
        ...
        return $this->httpRequest("v2/balance", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function positionADLQuantileEstimation(array $flags = [])
    {
        $opt = [];
        ...
        return $this->httpRequest("v1/adlQuantile", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function multipleOrder($batchOrders, array $flags = [])
    {   
        $parameters = [];
        $finalBatchOrders = [];
        if(is_array($batchOrders)){
            foreach ($batchOrders as $i => $order) {
                $opt = [];

                $order['type'] = $order['type'] ? $order['type'] : 'LIMIT';
                $opt = [
                    "symbol" => $order['symbol'],
                    "side" => $order['side'],
                    "type" => $order['type'],
                ...
                // someone has preformated there 8 decimal point double already
                // dont do anything, leave them do whatever they want
                if (gettype($order['price']) !== "string") {
                    // for every other type, lets format it appropriately
                    $price = number_format($order['price'], 8, '.', '');
                }
        
                if (is_numeric($order['quantity']) === false) {
                    // WPCS: XSS OK.
                    echo "warning: quantity expected numeric got " . gettype($order['quantity']) . PHP_EOL;
                }
        
                if (is_string($order['price']) === false) {
                    // WPCS: XSS OK.
                    echo "warning: price expected string got " . gettype($order['price']) . PHP_EOL;
                }
        
                if ($order['type'] === "LIMIT" || $order['type'] === "STOP_LOSS_LIMIT" || $order['type'] === "TAKE_PROFIT_LIMIT") {
                    $opt["price"] = $order['price'];
                    $opt["timeInForce"] = "GTC";
                }
        
                if (isset($order['flags']['stopPrice'])) {
                    $opt['stopPrice'] = $order['flags']['stopPrice'];
                }
        
                if (isset($order['flags']['icebergQty'])) {
                    $opt['icebergQty'] = $order['flags']['icebergQty'];
                }
        
                if (isset($order['flags']['newOrderRespType'])) {
                    $opt['newOrderRespType'] = $flags['newOrderRespType'];
                }
                

                array_push($finalBatchOrders, $opt);
            }
            $parameters['batchOrders'] = json_encode($finalBatchOrders);
        }

        if (isset($flags['recvWindow'])) {
            $parameters['recvWindow'] = $flags['recvWindow'];
        }

        return $this->httpRequest("v1/batchOrders", "POST", $parameters, true);
    }
     ...
     * @todo create documentation here
     */
    public function countdownCancelAll($symbol, $countDownTime, array $flags = [])
    {
        $opt = [
            "symbol" => $symbol,
        ...
        return $this->httpRequest("v1/countdownCancelAll", "POST", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function cancelMultipleOrder($symbol, $orderIdList, $isOrderIdListOrig, array $flags = [])
    {
        $opt = [
            "symbol" => $symbol
        ];
        ...
        if($isOrderIdListOrig){
            $opt['origClientOrderIdList'] = $orderIdList;
        }else{
            $opt['orderIdList'] = $orderIdList;
        }
        ...
        return $this->httpRequest("v1/batchOrders", "DELETE", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function queryOpenOrder($symbol, $isOrderIdOrig, array $flags = [])
    {
        $opt = [
            "symbol" => $symbol
        ];

        if (isset($flags['orderId'])) {
            if($isOrderIdOrig){
                $opt['origClientOrderId'] = $flags['orderId'];
            }else{
                $opt['orderId'] = $flags['orderId'];
            }
        }
        ...
        return $this->httpRequest("v1/openOrder", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function userForceOrders(array $flags = [])
    {
        $opt = [];
        ...
        if (isset($flags['autoCloseType'])) {
            $opt['autoCloseType'] = $flags['autoCloseType'];
        }
        ...
        return $this->httpRequest("v1/forceOrders", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function changeMarginType($symbol, $marginType, array $flags = [])
    {
        $opt = [
            "symbol" => $symbol,
        ...
        return $this->httpRequest("v1/marginType", "POST", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function changeInitialLeverage($symbol, $leverage, array $flags = [])
    {
        $opt = [
            "symbol" => $symbol,
            "leverage" => $leverage,
        ...
        return $this->httpRequest("v1/leverage", "POST", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function leverageBracketAll()
    {
        return $this->httpRequest("v1/leverageBracket");
    }
     ...
     * @todo create documentation here
     */
    public function modifyIsolatedPositionMargin($symbol, $amount, $type, array $flags = [])
    {
        $opt = [
            "symbol" => $symbol,
            "amount" => $amount,
        ...
        return $this->httpRequest("v1/positionMargin", "POST", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function getPositionMarginChangeHistory($symbol, array $flags = [])
    {
        $opt = [
            "symbol" => $symbol
        ];
        ...
        return $this->httpRequest("v1/positionMargin/history", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function changePositionMode($dualSidePosition, array $flags = [])
    {
        $opt = [
        ...
        return $this->httpRequest("v1/positionSide/dual", "POST", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function getCurrentPositionMode(array $flags = [])
    {
        $opt = [];
        ...
        return $this->httpRequest("v1/positionSide/dual", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function apiTradingStatus(array $flags = [])
    {
        $opt = [];
        ...
        return $this->httpRequest("v1/apiTradingStatus", "GET", $opt, true);
    }
     ...
     * @todo create documentation here
     */
    public function createNewListenKey()
    {
        return $this->httpRequest("v1/listenKey", "POST");
    }
     ...
     * @todo create documentation here
     */
    public function renewListenKey()
    {
        return $this->httpRequest("v1/listenKey", "PUT");
    }
    ...
    {
        return $this->httpRequest("v1/listenKey", "DELETE");
    }
}
